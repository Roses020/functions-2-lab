...
// CODE HERE
const totalPopulation   = populations.reduce((acumulator,current) => {
...
// CODE HERE
let postTaxOrders = orders.map(element =>{
...
let filteredPurchases = purchases.filter(element =>{
  return element.owner === 'Bob'
})

let mappedPurchases = filteredPurchases.map(element =>{
  return element.price
})

let bobsTotal = mappedPurchases.reduce((acumulator,current) =>{
  return acumulator + current
})
